...
fun SessionsScreen(
    ...
    LazyVerticalGrid ( // Created vertical grid
        modifier = modifier
            .fillMaxSize(),
        ...
        item {
            Row (
                ...
                dayChipItems.forEachIndexed { index, chipItem -> // this is button of days
                    FilterChip(
                        selected = selectedChipIndex == index, // For tracking state we need to create state
                        onClick = {
        ...
        items(uiState.sessionInfos) { // showing items in grid
            SessionItem(
                sessionInfo = it
            )
        }
...
data class DayChipItem( // Days data class
    val day: Day,
    val labelResourceId: Int
)

val dayChipItems = listOf( // List of created days
